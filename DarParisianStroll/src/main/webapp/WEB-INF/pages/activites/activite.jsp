...
<body
	...
	<jsp:include page="../header.jsp">
		<jsp:param name="page_active" value="activite" />
										...
										<c:when test="${fn:length(routeList) gt 3}">
											<p>
												<c:forEach begin="0" end="2" varStatus="loop">
													<a href="itineraire?id=${routeList[loop.index].route_id}">${routeList[loop.index].name}</a>
													<c:forEach begin="1" end="${reviewList[loop.index]}" varStatus="loop">
														<span>&#9733;</span>
													</c:forEach>
													<c:forEach begin="${reviewList[loop.index]}" end="4" varStatus="loop">
														<span>&#9734;</span>
													</c:forEach>
													<br>
												</c:forEach>
										...
										<c:otherwise>
											<p>
												<c:forEach begin="0" end="${fn:length(routeList)}"
													varStatus="loop">
												<a href="itineraire?id=${routeList[loop.index].route_id}">${routeList[loop.index].name}</a><br>
												</c:forEach>
											</p>
										</c:otherwise>
									</c:choose>
